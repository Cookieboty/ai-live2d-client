...
### 特殊字符路径问题修复
**问题描述**: makoto0模型配置文件中包含日文字符路径，导致404错误
- 错误路径: `"詩音マコト.moc"` 和 `"詩音マコト.2048/texture_00.png"`
- 实际文件: `makoto.moc` 和 `makoto.2048/texture_00.png`

**修复方案**:
1. 修正 `packages/renderer/public/assets/models/makoto0/makoto.model.json` 配置文件
2. 在 `customFetch` 函数中添加URL编码处理机制
3. 自动检测和处理包含非ASCII字符的URL路径
...
### 重复警告信息问题修复
**问题描述**: 某些模型（如makoto0）没有动作组配置，导致系统不断尝试播放动作并打印警告信息
- 控制台不断输出: `[Live2D] No motion groups available for model`
- 影响性能和用户体验

**根本解决方案**:
1. **完全禁用动作功能**: 对于没有动作组的模型，彻底禁用动作播放功能，而不是仅仅减少警告频率
2. **智能检测机制**: 在模型加载后检查是否有可用动作组，设置 `hasMotions` 标志
3. **条件执行**: 只有当 `hasMotions` 为 true 时才执行动作相关逻辑
...
## 备注
此次重构大幅简化了模型加载和换装的复杂度，提供了更好的可维护性和用户体验。新的结构更加直观，便于后续功能扩展。同时修复了特殊字符路径导致的加载问题，提升了系统的健壮性。 